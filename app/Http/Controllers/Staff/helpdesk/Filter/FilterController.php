<?php
namespace App\Http\Controllers\Staff\helpdesk\Filter;

use App\Http\Controllers\Staff\helpdesk\TicketController;
...
class FilterController extends Controller
{
    ...
    public function table()
    {
        ...
        $tickets = $ticket
            ...
            ->leftJoin('tickets_priorities', 'tickets_priorities.priority_id', '=', 'tickets.priority_id')
            ->leftJoin('users as u', 'u.id', '=', 'tickets.user_id')
            ->leftJoin('users as u1', 'u1.id', '=', 'tickets.assigned_to')
            ->leftJoin('tickets__attachments', 'tickets__attachments.thread_id', '=', 'ticket_thread.id')
            ->leftJoin('tickets__collaborators', 'tickets__collaborators.ticket_id', '=', 'tickets.id')
            ->select(
                'tickets.id',
                'ticket_thread.title',
                'tickets.ticket_number',
                'tickets_priorities.priority',
                ...
                'tickets_priorities.priority_color',
                DB::raw('COUNT(DISTINCT ticket_thread2.id) as countthread'),
                DB::raw('COUNT(tickets__attachments.thread_id) as countattachment'),
