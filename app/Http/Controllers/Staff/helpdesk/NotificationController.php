...
class NotificationController extends Controller
{
    ...
    public function send_notification()
    {
        ...
        if ($mailbox->notification_cron == 1) {
            ...
            // Send notification details to all the staff
    ...
    {
        // get all admin users
        $users = Staff::where('role', '=', 'admin')->get();
        foreach ($users as $user) {
            // Send notification details to admin
            $mailbox = $user->email;
            $user_name = $user->first_name . ' ' . $user->last_name;
            $view = View::make('mailboxes.notifications.admin', ['company' => $company, 'name' => $user_name]);
    ...
    public function send_notification_to_manager($company)
    {
        ...
        foreach ($depts as $dept) {
            if (isset($dept->manager)) {
                $dept_name = $dept->name;
                $users = Staff::where('id', '=', $dept->manager)->get();
                foreach ($users as $user) {
                    // Send notification details to manager of a department
                    $mailbox = $user->email;
                    $user_name = $user->first_name . ' ' . $user->last_name;
                    $view = View::make('mailboxes.notifications.manager', ['company' => $company, 'name' => $user_name]);
                    $contents = $view->render();
                    $from = $this->PhpMailController->mailfrom('1', '0');
                    $to = [
                        'name' => $user_name,
    ...
    public function send_notification_to_team_lead($company)
    {
        ...
        foreach ($teams as $team) {
            if (isset($team->team_lead)) {
                $team_name = $team->name;
                $users = Staff::where('id', '=', $team->team_lead)->get();
                foreach ($users as $user) {
                    // Send notification details to team lead
                    $mailbox = $user->email;
                    $user_name = $user->first_name . ' ' . $user->last_name;
                    $view = View::make('mailboxes.notifications.lead', ['company' => $company, 'name' => $user_name, 'team_id' => $team->id]);
                    $contents = $view->render();
                    $from = $this->PhpMailController->mailfrom('1', '0');
                    $to = [
                        'name' => $user_name,
    ...
    {
        // get all staff users
        $users = Staff::where('role', '=', 'staff')->get();
        foreach ($users as $user) {
            // Send notification details to all the staff
            $mailbox = $user->email;
            $user_name = $user->first_name . ' ' . $user->last_name;
            $view = View::make('mailboxes.notifications.staff', ['company' => $company, 'name' => $user_name, 'user_id' => $user->id]);
            $contents = $view->render();
            $from = $this->PhpMailController->mailfrom('1', '0');
            $to = [
                'name' => $user_name,
