<?php
namespace App\Http\Controllers\Staff\helpdesk\Notifications;
    //models
//classes
use DB;
use Lang;
...
class NotificationController extends Notification
{
    ...
    public function saveInThread($change, $model, $system = false)
    {
        ...
        if ($body && $this->save_in_thread) {
            $thread = \App\Model\helpdesk\Ticket\TicketThread::create([
                'thread_type' => $type,
                'body' => $body,
                'ticket_id' => $ticket_id,
    ...
    public function mobilePush($notification_id, $to)
    {
        ...
        where('id', $notification_id)
            ...
            ->first()
            ->toArray();
        \App\Staff::whereIn('id', $to)
            ->where('role', '!=', 'user')
            ->select('fcm_token', 'i_token')
            ->chunk(10, function ($staff) use ($noti, $fcm) {
    ...
    public function sendEmail()
    {
        ...
        if ($this->isMode('email') && $this->send_mail) {
            ...
            foreach ($mailboxes as $name => $mailbox) {
                //echo $name." => ".$mailbox."<br>";
                $this->postMail($mailbox, $name);
            }
        }
    }
    ...
    public function getField($field = 'email', $schma = true, $collect = false, $collection_fields = '')
    {
        ...
        if ($schma == true) {
            $unique = \App\Staff::whereNotNull($field)->whereIn('id', $unique)->pluck($field, 'first_name')->toArray();
        }
        if ($collect == true) {
            if (count($collection_fields) > 0) {
                $unique = \App\Staff::whereNotNull($field)->select($collection_fields)->whereIn('id', $unique)->get();
            } else {
                $unique = \App\Staff::whereNotNull($field)->whereIn('id', $unique)->get();
            }
        }
        return $unique;
    }
        ...
        switch ($person) {
            case 'department_members': // pass department id
                if ($ticket) {
                    ...
                    $staff = \App\Staff::where('primary_dpt', $modelid)
            ...
            case 'team_members': //pass team id
                if ($ticket) {
                    ...
                    $staff = \App\Model\helpdesk\Staff\Assign_team_Staff::
                ...
                return $staff;
            case 'staff':
                $staff = \App\Staff::where('role', 'staff')
                    ->where('active', 1)
                    ->where('ban', 0)
                    ->where('is_delete', 0)
                    ->select('id')
                ...
                return $staff;
            case 'admin':
                $staff = \App\Staff::where('role', 'admin')
                    ->where('active', 1)
                    ->where('ban', 0)
                    ->where('is_delete', 0)
                    ->select('id as admin')
            ...
            case 'user': // pass ticket user id
                ...
                return $staff;
            case 'agent_admin':
                $staff = \App\Staff::where('role', '!=', 'user')
                    ->where('active', 1)
                    ->where('ban', 0)
                    ->where('is_delete', 0)
            ...
            case 'department_manager'://pass department id
                if ($ticket) {
                    $modelid = $ticket->dept_id;
                    $staff = \App\Model\helpdesk\Staff\Department::where('department.id', $modelid)
            ...
            case 'team_lead': //pass team id
                if ($ticket) {
                    $modelid = $ticket->team_id;
                    $staff = \App\Model\helpdesk\Staff\Teams::where('teams.id', $modelid)
                        ->where('status', 1)
                ...
                return $staff;
            case 'relation_manager'://pass user id
                if ($ticket) {
                ...
                if ($modelid) {
                    ...
                    if ($org) {
                        $orgid = $org->org_id;
                        $staff = \App\Model\helpdesk\Agent_panel\Relation::where('id', $orgid)->select('head as relation_manager')->get()->toArray();
                    }
                }
                return $staff;
            ...
            case 'assigned_agent_team':
                if ($ticket) {
                    $assigned = $ticket->assigned()
                        ...
                        ->value('id')//                            ->get()
                    ;
                    $staff = ['assigned_agent_team' => $assigned];
                }
                return $staff;
            case 'all_department_manager':
                $staff = \App\Model\helpdesk\Staff\Department::
                ...
                return $staff;
            case 'all_team_lead':
                $staff = \App\Model\helpdesk\Staff\Teams::where('teams.status', 1)
                    ->join('users', function ($join) {
            ...
            case 'client':
                //dd($this);
                if ($ticket) {
                    $staff = ['client' => $ticket->user_id];
                } elseif ($this->userid) {
                    $staff = ['client' => $this->userid];
                }
                return $staff;
        }
    }
    ...
    public function sendSms()
    {
        if ($this->isMode('sms')) {
            ...
            if ($this->checkPluginSetup()) {
                ...
                logging('aler & notification', Lang::get('lang.can-not-send-message-sms-plugin-not-active'), 'info');
            }
        }
    }
    ...
    public function switchNewSchema($key, $value, $model, $auth_username)
    {
        switch ($key) {
            case 'priority_id':
                $priority = '';
                $schema = \App\Model\helpdesk\Ticket\TicketPriority::where('priority_id', $value)->select('priority')->first();
                if ($schema) {
            ...
            case 'source':
                $source = '';
                $schema = \App\Model\helpdesk\Ticket\TicketSource::where('id', $value)->select('name')->first();
                if ($schema) {
            ...
            case 'help_topic_id':
                $topic = '';
                $schema = \App\Model\helpdesk\Manage\HelpTopic::where('id', $value)->select('topic')->first();
                if ($schema) {
            ...
            case 'status':
                $status = '';
                $schema = \App\Model\helpdesk\Ticket\TicketStatus::where('id', $value)->select('name')->first();
                if ($schema) {
            ...
            case 'assigned_to':
                $assigned = '';
                $schema = \App\Staff::where('id', $value)->select('first_name', 'last_name', 'user_name')->first();
                if ($schema) {
            ...
            case 'dept_id':
                $department = '';
                $schema = \App\Model\helpdesk\Staff\Department::where('id', $value)->select('name')->first();
                if ($schema) {
    ...
    public function postMail($to_email, $to_name)
    {
        ...
        logging('aler & notification', 'Alert email has sent to ' . json_encode($to) . 'with ' . json_encode([$this->message, $this->variable]), 'info');
    }
