...
/**
 * StaffController
 * This controller is used to CRUD an User details, and proile management of an staff.
 *
...
class StaffController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $table = \ Datatable::table()
                ...
                ->noScript();
            return view('themes.default1.staff.helpdesk.user.index', compact('table'));
        } catch (Exception $e) {
    ...
    public function deletedUser()
    {
        try {
            // dd('here');
            /* get all values in Sys_user */
            return view('themes.default1.staff.helpdesk.user.deleteduser');
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function user_list(Request $request)
    {
        ...
        if ($type === 'staff') {
            $users = Staff::where('role', '=', 'staff')->where('is_delete', '=', 0);
        } elseif ($type === 'users') {
            $users = Staff::where('role', '=', 'user')->where('is_delete', '=', 0);
        } elseif ($type === 'active-users') {
            $users = Staff::where('role', '!=', 'admin')->where('active', '=', 1);
        } elseif ($type === 'inactive') {
            $users = Staff::where('role', '!=', 'admin')->where('active', '=', 0);
        } elseif ($type === 'deleted') {
            $users = Staff::where('role', '!=', 'admin')->where('is_delete', '=', 1);
        } elseif ($type === 'banned') {
            $users = Staff::where('role', '!=', 'admin')->where('ban', '=', 1);
        } else {
            $users = Staff::where('role', '!=', 'admin')->where('is_delete', '=', 0);
        }
        ...
        return \Datatables::of($users)
            ...
            ->addColumn('email', function ($model) {
                $mailbox = "<a href='" . route('user.show', $model->id) . "'>" . $model->email . '</a>';
                return $mailbox;
            })
            /* column phone */
    ...
    public function create(CountryCode $code)
    {
        try {
            ...
            $org = Relation::pluck('name', 'id')->toArray();
            return view('themes.default1.staff.helpdesk.user.create', compact('org', 'settings', 'email_mandatory'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
            ...
            if ($user->save() == true) {
                ...
                if ($request->input('send_email')) {
                    try {
                        ...
                        $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('1', '0'), $to = ['name' => $name, 'email' => $mailbox], $message = ['subject' => null, 'scenario' => 'registration-notification'], $template_variables = ['user' => $name, 'email_address' => $mailbox, 'user_password' => $password]);
                    } catch (Exception $e) {
    ...
    public function randomPostPassword($id, ChangepasswordRequest $request)
    {
        try {
            ...
            $this->PhpMailController->sendmail($from = $this->PhpMailController
                ->mailfrom('1', '0'), $to = ['name' => $name, 'email' => $mailbox], $message = ['subject' => null, 'scenario' => 'reset_new_password'], $template_variables = ['user' => $name, 'user_password' => $password]);
    ...
    public function changeRoleUser($id)
    {
        ...
        // return 'Staff Role Successfully Change to User';
    }
        ...
        if ($users->role == 'staff') {
            if ($delete_all == null) {
                ...
                if ($assign_to[0] == 'user') {
                    ...
                    // }
                    // if ($ticket_logic3 = TicketThread::where('user_id', '=', $id)->get()) {
                    //     $ticket_logic3 = TicketThread::where('user_id', '=', $id)->update(['user_id' => $assign_to[1]]);
                    // }
                    // if ($ticket_logic4 = User_org::where('user_id', '=', $id)->get()) {
                    //     $ticket_logic4 = User_org::where('user_id', '=', $id)->update(['user_id' => $assign_to[1]]);
                    // }
                    // $thread2 = TicketThread::where('ticket_id', '=', $ticket->id)->first();
                    ...
                    // Assign_team_Staff::where('staff_id', '=', $id)->update(['staff_id' => $assign_to[1]]);
                ...
                if ($delete_all) {
                    ...
                    // Assign_team_Staff::where('staff_id', '=', $id)->delete();
    ...
    public function show($id)
    {
        try {
            ...
            if (count($users) > 0) {
                return view('themes.default1.staff.helpdesk.user.show', compact('users', 'policy'));
            } else {
    ...
    public function edit($id, CountryCode $code)
    {
        try {
            ...
            $orgs = Relation::all();
            // dd($org);
            $relation_id = User_org::where('user_id', '=', $id)->pluck('org_id')->first();
            // $org_name=Relation::where('id','=',$org_id)->pluck('name')->first();
            // dd($org_name);
            return view('themes.default1.staff.helpdesk.user.edit', compact('users', 'orgs', '$settings', '$email_mandatory', 'relation_id'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    /**
     * Assigning an user to an relation.
     *
     * @param type $id
     *
     * @return type boolean
    ...
    public function UserAssignOrg($id)
    {
        ...
        if ($org_name) {
            $org = Relation::where('name', '=', $org_name)->pluck('id')->first();
            if ($org) {
    ...
    /**
     * creating an relation in user profile page via modal popup.
     *
     * @param type $id
     *
     * @return type
    ...
    public function User_Create_Org($id)
    {
        ...
        if (\Input::get('name') == null) {
            ...
            // storing relation details and assigning the current user to that relation
            $org = new Relation();
    ...
    {
        try {
            return view('themes.default1.staff.helpdesk.user.export');
        } catch (Exception $ex) {
    ...
    public function getUsers($first, $last)
    {
        ...
        $users = $user->leftJoin('user_assign_organization', 'users.id', '=', 'user_assign_organization.user_id')
            ->leftJoin('relation', 'user_assign_organization.org_id', '=', 'relation.id')
            ->whereBetween('users.created_at', [$first, $last])
            ->where('role', 'user')
            ->where('active', 1)
            ->select('users.user_name as Username', 'users.email as Mailboxes', 'users.first_name as Fisrtname', 'users.last_name as Lastname', 'relation.name as Relation')
            ->get()
            ->toArray();
        return $users;
    }
    ...
    {
        $users = Staff::where('role', '<>', 'user')->where('active', '=', 1)->orderBy('first_name')->get();
    ...
    public function settingsUpdateStatus(Request $request)
    {
        try {
            ...
            Staff::where('id', $user_id)->update(['active' => $user_status]);
    ...
    public function settingsUpdateBan(Request $request)
    {
        try {
            ...
            $user = Staff::where('id', $user_id)->update(['ban' => $user_ban]);
            //            $spam_status_type = \App\Model\helpdesk\Ticket\TicketStatus::whereHas('type', function($query) {
    ...
    public function settingsUpdateMobileVerify(Request $request)
    {
        try {
            ...
            $user_id = $request->user_id;
            $user_ban = $request->settings_ban;
            Staff::where('id', $user_id)->update(['mobile_verify' => $user_ban]);
