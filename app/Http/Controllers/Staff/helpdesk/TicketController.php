...
use App\Model\helpdesk\Ticket\TicketCollaborator;
...
class TicketController extends Controller
{
    ...
    public function inbox_ticket_list()
    {
        $table = \Datatable::table()
            ...
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.inbox', compact('table'));
    }
    ...
    public function open_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.open', compact('table'));
    }
    ...
    public function answered_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.answered', compact('table'));
    }
    ...
    public function myticket_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.myticket', compact('table'));
    }
    ...
    public function overdue_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.overdue', compact('table'));
    }
    ...
    public function dueTodayTicketlist()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.duetodayticket', compact('table'));
    }
    ...
    public function closed_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.closed', compact('table'));
    }
    ...
    public function assigned_ticket_list()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.assigned', compact('table'));
    }
    ...
    public function newticket(CountryCode $code)
    {
        ...
        return view('themes.default1.staff.helpdesk.ticket.new');
    }
    ...
    public function post_newticket(Request $request, CountryCode $code, $api = false)
    {
        ...
        try {
            ...
            $phone = '';
            $source = TicketSource::where('name', '=', 'staff')->first();
            $headers = null;
            $auto_response = 0;
            $sla = '';
            $attach = $request->input('attachment');
            $inline = $request->input('inline');
            $result = $this->create_user($mailbox, $fullname, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source->id, $headers, $help->department, $assignto, $form_data, $auto_response, $status, $attach, $inline);
            if ($result[1]) {
    ...
    public function thread($id)
    {
        ...
        if ($auth_agent->role == 'staff') {
            ...
            $thread = TicketThread::where('ticket_id', '=', $id)->first();
        ...
        return view('themes.default1.staff.helpdesk.ticket.timeline', compact('tickets', 'max_size_in_bytes', 'max_size_in_actual', 'tickets_approval'), compact('thread', 'avg_rating', 'ticket_policy'));
    }
    ...
    public function reply(Request $request, $ticketid = '', $mail = true, $system_reply = true, $user_id = '')
    {
        ...
        try {
            ...
            \Event::fire(new \App\Events\ClientTicketFormPost($form_data, $mailbox, $source));
    ...
    public function replyNotification($ticket, $thread, $mail)
    {
        ...
        $collaborators = TicketCollaborator::where('ticket_id', '=', $ticketid)->get();
        if (!$mailbox) {
            $mail = false;
        }
        ...
        $notifications[] = [
            $key => [
                ...
                'variable' => ['ticket_number' => $ticket_number,
                    'user' => $username, 'agent_sign' => $staffign,],
                'ticketid' => $ticket->id,
                'send_mail' => $mail,
                'model' => $thread,
                'thread' => $thread,
     ...
     * @return type bool
     */
    public function ticketEditPost($ticket_id, TicketThread $thread, Tickets $ticket)
    {
        ...
        try {
            ...
            // dd($tkt_dept->dept_id);
            $priority = Input::get('tickets_priorities');
            $ticket->help_topic_id = Input::get('help_topic');
            // $dept = HelpTopic::select('department')->where('id', '=', $ticket->help_topic_id)->first();
            // $dept = $tkt_dept->dept_id;
            $sla = $this->getSla(Input::get('ticket_type'), $ticket->user_id, $tkt_dept, Input::get('tickets_sources'), $priority);
    ...
    public function ticket_print($id)
    {
        ...
        $html = view('themes.default1.staff.helpdesk.ticket.pdf', compact('id', 'ticket', 'tickets'))->render();
     ...
     * @return type bool
     */
    public function checkEmail($mailbox)
    {
        $check = Staff::where('email', '=', $mailbox)->orWhere('user_name', $mailbox)->orWhere('mobile', $mailbox)->first();
        if ($check == true) {
            return $check;
        }
        return false;
    ...
    {
        $check = Staff::where('mobile', '=', $mobile)->first();
        if (count($check) > 0) {
            return true;
        }
        return false;
    ...
    public function create_user($emailadd, $username, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, $auto_response, $status, $attach = [], $inline = [], $email_content = [])
    {
        ...
        $threaddata = TicketThread::where('ticket_id', '=', $ticketdata->id)->first();
        ...
        if ($ticket_number2) {
            if ($is_reply == 0) {
                ...
                $this_thread = TicketThread::where('ticket_id', '=', $ticketdata->id)->where('is_internal', 0)->orderBy('id', 'DESC')->first();
                $mail = 'ticket-reply-staff';
    ...
    public function check_ticket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $attach = [], $inline = [], $email_content = [])
    {
        ...
        if ($ticket) {
            ...
            if ($ticket->status > 1) {
                ...
                $ticket->save();
                $ticket_status = TicketStatus::where('id', '=', 1)->first();
                $user_name = Staff::where('id', '=', $user_id)->first();
                if ($user_name->role == 'user') {
                    $username = $user_name->user_name;
                } elseif ($user_name->role == 'staff' or $user_name->role == 'admin') {
                    $username = $user_name->first_name . ' ' . $user_name->last_name;
                }
                // $ticket_threads = new TicketThread();
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $attach = [], $inline = [])
    {
        ...
        $user_status = Staff::select('active')->where('id', '=', $user_id)->first();
    ...
    {
        if (!$status) {
            $requester = Staff::where('id', $requester_id)->first();
    ...
    public function close($id, Tickets $ticket, $api = true)
    {
        ...
        $ticket_status_message = TicketStatus::where('id', '=', $ticket_status->status)->first();
        $thread = new TicketThread();
        ...
        try {
            $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('0', $ticket_status->dept_id), $to = ['name' => $user_name, 'email' => $mailbox], $message = ['subject' => $ticket_subject . '[#' . $ticket_number . ']', 'scenario' => 'close-ticket'], $template_variables = ['ticket_number' => $ticket_number]);
        } catch (\Exception $e) {
            return 0;
        }
        $data = [
    ...
    public function delete($id, Tickets $ticket, $api = true)
    {
        ...
        if ($ticket_delete->status == 5) {
            ...
            $ticket_threads = TicketThread::where('ticket_id', '=', $id)->get();
            foreach ($ticket_threads as $ticket_thread) {
                $ticket_thread->delete();
            }
            $ticket_attachments = TicketAttachments::where('ticket_id', '=', $id)->get();
            ...
            $ticket_delete->save();
            $ticket_status_message = TicketStatus::where('id', '=', $ticket_delete->status)->first();
            $thread = new TicketThread();
    ...
    public function ban($id, Tickets $ticket, $api = true)
    {
        ...
        $user = Staff::where('id', '=', $ban_email)->first();
        $user->ban = 1;
        $user->save();
        $Email = $user->email;
    ...
    public function assign($id, $api = true)
    {
        ...
        foreach ($ticket_array as $id) {
            ...
            if ($assign_to[0] == 'team') {
                ...
                $ticket_number = $ticket->ticket_number;
                $ticket->save();
                $ticket_thread = TicketThread::where('ticket_id', '=', $id)->first();
                $ticket_subject = $ticket_thread->title;
                $thread = new TicketThread();
                ...
                if ($user_detail === null) {
                    $user_detail = Staff::where('id', '=', $assign_to[1])->first();
    ...
    public function storeCollaborators($headers, $id)
    {
        $company = $this->company();
        if (isset($headers)) {
            foreach ($headers as $mailbox => $name) {
                if ($name == null) {
                    $name = $mailbox;
                }
                $name = $name;
                $mailbox = $mailbox;
                if ($this->checkEmail($mailbox) == false) {
                    $create_user = new Staff();
                    $create_user->first_name = $name;
                    $create_user->user_name = $mailbox;
                    $create_user->email = $mailbox;
                    ...
                    try {
                        $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('1', '0'), $to = ['name' => $name, 'email' => $mailbox], $message = ['subject' => 'password', 'scenario' => 'registration-notification'], $template_variables = ['user' => $name, 'email_address' => $mailbox, 'user_password' => $password]);
                    } catch (\Exception $e) {
                    }
                } else {
                    $user = $this->checkEmail($mailbox);
                    $user_id = $user->id;
                }
                $collaborator_store = new TicketCollaborator();
    ...
    public function trash()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.trash', compact('table'));
    }
    ...
    public function unassigned()
    {
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.ticket.unassigned', compact('table'));
    }
    ...
    {
        $term = \Input::get('term');
        $user = \App\Staff::where('email', 'LIKE', '%' . $term . '%')->pluck('email');
     ...
     * @return type json
     */
    public function autosearch2(Staff $user)
    {
    ...
    {
        $mailbox = Input::get('search');
        $ticket_id = Input::get('ticket_id');
        $data = Staff::where('email', '=', $mailbox)->first();
        if ($data == null) {
            return '<div id="alert11" class="alert alert-warning alert-dismissable">'
            . '<button id="dismiss11" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'
            . '<i class="icon fa fa-ban"></i>'
            . 'This Mailboxes doesnot exist in the system'
            . '</div>'
            . '</div>';
        }
        $tickets__collaborators = TicketCollaborator::where('ticket_id', '=', $ticket_id)->where('user_id', '=', $data->id)->first();
        if (!isset($tickets__collaborators)) {
            $tickets__collaborators = new TicketCollaborator();
    ...
    {
        $name = Input::get('name');
        $mailbox = Input::get('email');
        $validator = \Validator::make(
            ['email' => $mailbox, 'name' => $name], ['email' => 'required|email']
        );
        if ($validator->fails()) {
            return 'Invalid email address.';
        }
        $ticket_id = Input::get('ticket_id');
        $user_search = Staff::where('email', '=', $mailbox)->first();
    ...
    {
        $id = Input::get('data1');
        $tickets__collaborators = TicketCollaborator::where('id', '=', $id)->delete();
        return 1;
    }
        ...
        if (Input::has('select_all')) {
            ...
            foreach ($selectall as $delete) {
                ...
                if ($value == 'Delete') {
                    ...
                    $notification->delete();
                    $thread = TicketThread::where('ticket_id', '=', $ticket->id)->get();
                    foreach ($thread as $th_id) {
                        // echo $th_id->id." ";
                        $attachment = TicketAttachments::where('thread_id', '=', $th_id->id)->get();
                        if (count($attachment)) {
                            foreach ($attachment as $a_id) {
                                // echo $a_id->id . ' ';
                                $attachment = TicketAttachments::find($a_id->id);
                                $attachment->delete();
                            }
                            // echo "<br>";
                        }
                        $thread = TicketThread::find($th_id->id);
                        //                        dd($thread);
                        $thread->delete();
                    }
                    $collaborators = TicketCollaborator::where('ticket_id', '=', $ticket->id)->get();
                    if (count($collaborators)) {
                        foreach ($collaborators as $collab_id) {
                            // echo $collab_id->id;
                            $collab = TicketCollaborator::find($collab_id->id);
                            $collab->delete();
                        }
                    }
    ...
    public function deptopen($id)
    {
        ...
        if (Auth::user()->role == 'staff') {
            if (Auth::user()->primary_dpt == $dept->id) {
                return view('themes.default1.staff.helpdesk.dept-ticket.tickets', compact('id'));
            } else {
    ...
    public function deptTicket($dept, $status)
    {
        ...
        $table = \Datatable::table()
            ->addColumn(
                '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
            ->noScript();
        return view('themes.default1.staff.helpdesk.dept-ticket.tickets', compact('dept', 'status', 'table'));
    }
    ...
    public function deptclose($id)
    {
        ...
        if (Auth::user()->role == 'staff') {
            if (Auth::user()->primary_dpt == $dept->id) {
                return view('themes.default1.staff.helpdesk.dept-ticket.closed', compact('id'));
            } else {
                return redirect()->back()->with('fails', 'Unauthorised!');
            }
        } else {
    ...
    public function deptinprogress($id)
    {
        ...
        if (Auth::user()->role == 'staff') {
            if (Auth::user()->primary_dpt == $dept->id) {
                return view('themes.default1.staff.helpdesk.dept-ticket.inprogress', compact('id'));
            } else {
                return redirect()->back()->with('fails', 'Unauthorised!');
            }
        } else {
    ...
    {
        $mailbox = MailboxSettings::where('id', '=', '1')->first();
        return $mailbox->sys_email;
    }
    ...
    public function checkLock($id)
    {
        ...
        if ($diff < $cad && Auth::user()->id != $ticket->lock_by) {
            $user_data = Staff::select('user_name', 'first_name', 'last_name')->where('id', '=', $ticket->lock_by)->first();
    ...
    public function changeOwner($id)
    {
        ...
        if ($action === 'change-add-owner') {
            $name = Input::get('name');
            $returnValue = $this->changeOwnerAdd($mailbox, $name, $ticket_id);
            if ($returnValue === 0) {
                return 4;
        ...
        $user = Staff::where('email', '=', $mailbox)->first();
        $count = count($user);
        if ($count === 1) {
            $user_id = $user->id;
            $ticket = Tickets::where('id', '=', $id)->first();
            ...
            if (Alert::first()->internal_status == 1 || Alert::first()->internal_assigned_agent == 1) {
                ...
                Mail::send('mailboxes.Ticket_assign', ['staff' => $agent, 'ticket_number' => $ticket_number, 'from' => $company, 'master' => $master, 'system' => $system], function ($message) use ($agent_email, $agent, $ticket_number, $ticket_subject) {
     ...
     * @return type json
     */
    public function changeOwnerAdd($mailbox, $name, $ticket_id)
    {
        $name = $name;
        $mailbox = $mailbox;
        $ticket_id = $ticket_id;
    ...
    public function getMergeTickets($id)
    {
        if ($id == 0) {
            ...
            foreach ($t_id as $value) {
                $title = TicketThread::select('title')->where('ticket_id', '=', $value)->first();
            ...
            foreach ($ticket_data as $value) {
                $title = TicketThread::select('title')->where('ticket_id', '=', $value->id)->first();
    ...
    public function mergeTickets($id)
    {
        ...
        foreach ($t_id as $value) {//to create new thread of the tickets to be merged with parent
            ...
            if (!empty(Input::get('title'))) {
                TicketThread::where('ticket_id', '=', $p_id)->first()
                    ->update(['title' => Input::get('title')]);
            }
            $new_thread = new TicketThread();
            ...
            $new_parent_thread = new TicketThread();
    ...
    {
        $title = TicketThread::select('title')->where('ticket_id', '=', $id)->first();
    ...
    {
        return view('themes.default1.staff.helpdesk.ticket.getautocomplete');
    }
    ...
    public function pdfThread($threadid)
    {
        try {
            ...
            $html = view('themes.default1.staff.helpdesk.ticket.thread-pdf', compact('thread', 'system', 'company', 'ticket'))->render();
            $html1 = mb_convert_encoding($html, 'HTML-ENTITIES', 'UTF-8');
            return PDF::load($html1)->show();
        } catch (Exception $ex) {
        ...
        try {
            $table = \Datatable::table()
                ->addColumn(
                    '', Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'), Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'), Lang::get('lang.created-at'))
                ->noScript();
            return view('themes.default1.staff.helpdesk.followup.followup', compact('table'));
        } catch (Exception $e) {
            return Redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function sendMergeNotification($p_id, $t_id)
    {
        try {
            ...
            $user_detail = Staff::where('id', '=', $ticket_details->user_id)->first();
