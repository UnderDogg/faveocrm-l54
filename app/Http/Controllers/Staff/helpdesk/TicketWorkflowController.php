...
class TicketWorkflowController extends Controller
{
    ...
    {
        $contact_details = ['email' => $fromaddress, 'mailbox_name' => $fromname, 'subject' => $subject, 'message' => $body];
        ...
        foreach ($workflows as $workflow) {
            ...
            if ($workflow) {
                ...
                foreach ($worklfow_rules as $worklfow_rule) {
                    ...
                    } elseif ($worklfow_rule->matching_scenario == 'mailbox_name') {
                        if ($rule_condition = $this->checkRuleCondition($contact_details['mailbox_name'], $worklfow_rule->matching_relation, $worklfow_rule->matching_value) == true) {
                            $ticket_settings_details = $this->applyActionCondition($workflow->id, $ticket_settings_details);
                        }
                    ...
                    foreach ($worklfow_rules as $worklfow_rule) {
                        if ($worklfow_rule) {
                            ...
                            if ($worklfow_rule->matching_scenario == 'email') {
                                ...
                                if ($this->checkRuleCondition($contact_details['mailbox_name'], $worklfow_rule->matching_relation, $worklfow_rule->matching_value) == true) {
                                    $ticket_settings_details = $this->applyActionCondition($workflows_web->id, $ticket_settings_details);
                                }
                            } elseif ($worklfow_rule->matching_scenario == 'subject') {
        ...
        if ($source == 2) {
            // get all the workflow which are applied to ticket generated via mailboxes and in ascending order
    ...
    {
        $priority = TicketPriority::where('priority_id', '=', $workflow_action->action)->first();
        if ($priority == null) {
            return $ticket_settings_details;
        } else {
    ...
    /**
     * function to assing a ticket to an staff.
     *
     * @param type $workflow_action
     * @param type $ticket_settings_details
     *
    ...
    {
        $agent = Staff::where('id', '=', $workflow_action->action)->where('role', '!=', 'user')->first();
        if ($agent == null) {
            return $ticket_settings_details;
        } else {
    ...
    {
        $help_topic = HelpTopic::where('id', '=', $workflow_action->action)->first();
        if ($help_topic == null) {
            return $ticket_settings_details;
        } else {
    ...
    {
        $status = TicketStatus::where('id', '=', $workflow_action->action)->first();
        if ($status == null) {
            return $ticket_settings_details;
        } else {
