...
@section('breadcrumbs')
  ...
  @section('content')
    <!-- open a form -->
  {!!Form::model($mailboxes,['url'=>'','id'=>'form'])!!}
  <input type="hidden" name="_token" value="{{ csrf_token() }}">
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">{!! Lang::get('lang.mailbox_information_and_settings') !!}</h3>
    </div>
    <div class="box-body">
      <div id="head"></div>
    ...
    <div class="box-body">
      <div class="row">
        ...
        <!-- Mailboxes name -->
        <div class="col-xs-4 form-group {!! $errors->has('mailbox_name') ? 'has-error' : ''!!}" id="email_name_error">
          {!! Form::label('mailbox_name',Lang::get('lang.from_name')) !!} <span class="text-red"> *</span>
          {!! $errors->first('mailbox_name', '<spam class="help-block">:message</spam>') !!}
          {!! Form::text('mailbox_name',null,['class' => 'form-control', 'id' => 'mailbox_name']) !!}
        </div>
        <!-- password -->
        <div class="col-xs-4 form-group {!! $errors->has('password') ? 'has-error' : ''!!}" id="password_error">
          ...
          <input type="password" name="password" class="form-control" id="password" value={!! $mailboxes->password !!} >
        </div>
      </div>
    </div>
    <div class="box-header with-border">
        ...
        <div class="form-group">
          ...
          <div class="col-xs-2 form-group">
            ...
            <input type="checkbox" name="fetching_status" id="fetching_status" <?php
              if ($mailboxes->fetching_status == 1) {
                echo "checked='checked'";
              }
      ...
      <div class="row">
        ...
        <div class="col-xs-2 form-group {!! $errors->has('fetching_encryption') ? 'has-error' : ''!!}"
          ...
          <select name="fetching_encryption" class='form-control' id='fetching_encryption'>
            ...
            <option <?php
                    if ($mailboxes->fetching_encryption == 'ssl' || $mailboxes->fetching_encryption === 'ssl') {
                      echo 'selected="selected"';
                    }
                    ?> value="ssl">SSL
            </option>
            <option <?php
                    if ($mailboxes->fetching_encryption == 'tls' || $mailboxes->fetching_encryption === 'tls') {
                      echo 'selected="selected"';
                    }
                    ?> value="tls">TLS
            </option>
            <option <?php
                    if ($mailboxes->fetching_encryption == 'starttls' || $mailboxes->fetching_encryption === 'starttls') {
                      echo 'selected="selected"';
                    }
    ...
    <div class="box-body">
      ...
      <div class="row">
        ...
        <div class="col-xs-2 form-group {!! $errors->has('sending_protocol') ? 'has-error' : ''!!}"
          ...
          {!!Form::select('sending_protocol',[''=>'Select','Drives'=>$services],$mailboxes->getCurrentDrive(),['class' => 'form-control select','id'=>'service']) !!}
        </div>
        <!-- sending hoost -->
    ...
    <div class="box-footer">
      <div class="form-group">
        <input type="checkbox" name="sys_email" @if($sys_email->sys_email == $mailboxes->id) checked
               @endif @if($count > 1 && $sys_email->sys_email == $mailboxes->id) disabled @endif
  ...
  <script type="text/javascript">
    ...
    $('#form').on('submit', function () {
      ...
      $.ajax({
        type: "POST",
        url: "{!! route('validating.email.settings.update', $mailboxes->id ) !!}",
        dataType: "json",
        data: form_data,
        headers: {
    ...
    $(document).ready(function () {
      ...
      function send(serviceid) {
        $.ajax({
          ...
          dataType: "html",
          data: {'service': serviceid, 'emailid':{{$mailboxes->id}}},
