      ...
      @if(Session::has('success'))
        ...
        <table id="example1" class="table table-bordered table-striped">
          ...
          @foreach($directories as $dir)
            <?php if ($dir === '.' or $dir === '..' or $dir === 'notifications')
              continue; ?>
            <?php //dd($directory); ?>
            <tr>
              <!-- Template Name with Link to Edit page along Id -->
              <td><a
                  href="{{route('template.list',[$dir,$directory])}}"><?php $parts = explode('.',$dir); $names  = $parts[0]; $name = str_replace('_', ' ', $names);$name1 = ucfirst($name); echo $name1?></a>
              </td>
              <!-- template Status : if status==1 active -->
              <!-- Deleting Fields -->
              <?php $status = DB::table('mailboxes__settings')->first();  ?>
              ...
              <td>
                ...
                <button class="btn btn-danger btn-xs {!! $dis !!}" data-toggle="modal" data-target="#{{$dir}}delete">
                  Delete
                </button>
