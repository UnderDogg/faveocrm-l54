          ...
          @if(Session::has('success'))
            ...
            <table id="example1" class="table table-bordered table-striped">
              ...
              @foreach($templates as $template)
                <?php if ($template === '.' or $template === '..')
                  continue; ?>
                <tr>
                  <!-- Template Name with Link to Edit page along Id -->
                  <td><a
                      href="{{route('template.read',[$template,$directory])}}"><?php $parts = explode('.', $template); $names = $parts[0]; $name = str_replace('-', ' ', $names); $cname = ucfirst($name); echo $cname?></a>
                  </td>
