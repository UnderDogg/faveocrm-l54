          ...
          <tr>
            ...
            <td>
              <a href="{!! route('status.edit',$status->id) !!}">
                <button class="btn btn-info btn-xs"><i class='fa fa-edit'> </i> {!! Lang::get('lang.edit') !!}</button>
              </a>
              <button class="btn btn-danger btn-xs" <?php if ($status->default == 1 || $status->id == 6) {
                echo "disabled='disabled'";
              } ?> data-toggle="modal" data-target="#{{$status->id}}delete"><i
                  class='fa fa-trash'> </i> {!! Lang::get('lang.delete') !!}</button>
