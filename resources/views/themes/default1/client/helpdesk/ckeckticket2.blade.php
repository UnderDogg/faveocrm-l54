...
@section('breadcrumb')
  <div class="site-hero clearfix">
    <ol class="breadcrumb breadcrumb-custom">
      ...
      <li class="active">{!! Lang::get('lang.tickets__statuses') !!}</li>
    </ol>
  </div>
@stop
...
@section('content')
        ...
        <div class="col-md-3">
          <div class="pull-right">
            ...
            <button type="button" class="btn btn-default"><i class="fa fa-print" style="color:blue;"> </i> {!!
              link_to_route('ticket.print','Print',[$tickets->id]) !!}
            </button>
            --}}
            <!-- </div> -->
            <div class="btn-group">
              ...
              <ul class="dropdown-menu">
                <li><a href="#" id="open"><i class="fa fa-folder-open" style="color:#FFD600;"> </i>{!!
                    Lang::get('lang.open') !!}</a></li>
                <li><a href="#" id="close"><i class="fa fa-check" style="color:#15F109;"> </i>{!!
                    Lang::get('lang.close') !!}</a></li>
                <li><a href="#" id="resolved"><i class="fa fa-check-circle " style="color:#0EF1BE;"> </i> {!!
  ...
  $attachment = App\Model\helpdesk\Ticket\TicketAttachments::where('thread_id', '=', $conversation->id)->first();
  if ($attachment == null ) {
    $body = $conversation->body;
  } else {
  $body = $conversation->body;
  $attachments = App\Model\helpdesk\Ticket\TicketAttachments::where('thread_id', '=', $conversation->id)->orderBy('id', 'DESC')->get();
      ...
      <article class="comment-body">
        <footer class="comment-meta"
          ...
          <div class="comment-author">
            @if($role->profile_pic)
              <img src="{{asset('uploads/profilepic')}}{{'/'}}{{$role->profile_pic}}" alt="" height="50" width="50"
          ...
          <div class="comment-metadata">
            <small class="date text-muted">
              <time datetime="2013-10-23T01:50:50+00:00"><i
                  class="fa fa-clock-o"> </i> {{ faveoDate($conversation->created_at) }}</time>
            </small>
        ...
        <div class="timeline-footer" style="margin-bottom:-5px">
          ...
          <ul class='mailbox-attachments clearfix'>
            ...
            $data = ob_get_contents();
            ob_end_clean();
            $var = '<a href="'.URL::route('image', array('image_id' => $attachment->id)).'" target="_blank"><img
                style="max-width:200px;height:133px;" src="data:image/jpg;base64,' . base64_encode($data)  . '"/></a>';


            echo '
            <li style="background-color:#f4f4f4;"><span class="mailbox-attachment-icon has-img">'.$var.'</span>
              <div class="mailbox-attachment-info"><b style="word-wrap: break-word;">'.$attachment->name.'</b><br/>
                <p>'.$value.'</p></div>
            </li>
            ';
            }
            ...
            {
            $var = '<a style="max-width:200px;height:133px;color:#666;"
                       href="'.URL::route('image', array('image_id' => $attachment->id)).'" target="_blank"><span
                class="mailbox-attachment-icon" style="background-color:#fff;">'.strtoupper($attachment->type).'</span>
              <div class="mailbox-attachment-info"><span><b
                    style="word-wrap: break-word;">'.$attachment->name.'</b><br/><p>'.$value.'</p></span></div>
            </a>';
            echo '
            <li style="background-color:#f4f4f4;">'.$var.'</li>
            ';
            }
            }
            }
