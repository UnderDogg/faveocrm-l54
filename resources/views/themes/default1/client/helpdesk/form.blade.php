    ...
    <div class="banner-wrapper  clearfix" style="border-color : <?php echo $portal->client_header_color; ?>">
      ...
      <div class="banner-content text-center">
        ...
        <br/><input type="submit" value="{!! Lang::get('lang.check_ticket_status') !!}" class="btn btn-custom btn-info"
                    style="background-color : <?php echo $portal->client_header_color; ?>;border-color : <?php echo $portal->client_header_color; ?>">
        {!! Form::close() !!}
      </div>
    </div>
        ...
        <div class="container">
            ...
            <ng-form name="faveoClientForm">
              <div class="row" style="margin:15px;width:100%" ng-if="node.customerDisplay">
                <div class="col-sm-3" style="padding: 0px;line-height: 2.5">
                  <label>@{{node.label | translate}}</label><span ng-show="node.customerRequiredFormSubmit"
                                                                  style="color:red">*</span>
                </div>
                <div class="col-sm-8" style="padding: 0px">
                  <input type="text" name="textfield@{{$index}}" ng-if="node.type=='text'&&node.customerDisplay"
                         class="form-control" style="border-radius: 0px;width:85%" ng-model="node.value"
                         ng-required="@{{node.customerRequiredFormSubmit}}">
                <span style="color:red"
                      ng-show="faveoClientForm.textfield@{{$index}}.$dirty && faveoClientForm.textfield@{{$index}}.$invalid">
                                          <span ng-show="faveoClientForm.textfield@{{$index}}.$error.required">@{{node.label | translate}}
                                            is required.</span>
                </span>
                  <input type="text" name="numberfield@{{$index}}" ng-if="node.type=='number'&&node.customerDisplay"
                         class="form-control numberOnly" style="border-radius: 0px;width:85%" ng-model="node.value"
                         ng-required="@{{node.customerRequiredFormSubmit}}">
                <span style="color:red"
                      ng-show="faveoClientForm.numberfield@{{$index}}.$dirty && faveoClientForm.numberfield@{{$index}}.$invalid">
                                          <span ng-show="faveoClientForm.numberfield@{{$index}}.$error.required">@{{node.label | translate}}
                                            is required.</span>
                </span>
                  <input type="text" name="datefield@{{$index}}" ng-if="node.type=='date'" class="form-control"
                         style="border-radius: 0px;width:85%"
                         ng-pattern="/^[0,1]?\d{1}\/(([0-2]?\d{1})|([3][0,1]{1}))\/(([1]{1}[9]{1}[9]{1}\d{1})|([2-9]{1}\d{3}))$/"
                         ng-required="@{{node.customerRequiredFormSubmit}}" placeholder="MM/DD/YYYY"
                         ng-model="node.value">
                <span style="color:red"
                      ng-show="faveoClientForm.datefield@{{$index}}.$dirty && faveoClientForm.datefield@{{$index}}.$invalid">
                                          <span ng-show="faveoClientForm.datefield@{{$index}}.$error.required">@{{node.label | translate}}
                                            is required.</span>
                  ...
                  <div class="input-group" ng-if="node.type=='email'&& node.label=='Requester'" style="width:100%">
                    <input type="text" class="form-control" style="border-radius: 0px;width:85%" ng-model="node.value"
                           id="clientreq" onchange="angular.element(this).scope().clientUserName(this)"
                           placeholder="Requester Username" ng-required="@{{node.customerRequiredFormSubmit}}">
                  ...
                  <div ng-if="newReqField && node.type=='email'&& node.label=='Requester'" style="margin-top:15px;">
                    <input type="text" name="requsName@{{$index}}" class="form-control"
                           style="border-radius:0;margin-top:10px;width:85%" ng-model="req.name"
                           placeholder="New Requester Name" id="requesterName" ng-required="@{{node.customer_name}}"/>
                                <span style="color:red"
                                      ng-show="faveoClientForm.requsName@{{$index}}.$dirty && faveoClientForm.requsName@{{$index}}.$invalid">
                                          <span ng-show="faveoClientForm.requsName@{{$index}}.$error.required">@{{node.label | translate}}
                                            is required.</span>
                                </span>
                    ...
                    <div class="row" style="width:85%">
                      ...
                      <div class="col-sm-9">
                        <input type="text" name="requsMobile@{{$index}}" class="form-control"
                               style="border-radius:0;margin-top:10px" ng-model="req.mobile"
                               placeholder="New Requester mobile" ng-pattern="/^[0-9]{1,99}$/" id="requesterMobile"
                               ng-required="@{{node.customer_mobile}}"/>
                                <span style="color:red"
                                      ng-show="faveoClientForm.requsMobile@{{$index}}.$dirty && faveoClientForm.requsMobile@{{$index}}.$invalid">
                  ...
                  <textarea name="descript@{{$index}}" class="form-control"
                            ng-if="node.type=='textarea'&& node.default=='no'" style="border-radius: 0px;width:85%"
                            ng-model="node.value" ng-required="@{{node.customerRequiredFormSubmit}}"></textarea>
                <span style="color:red"
                      ng-show="faveoClientForm.descript@{{$index}}.$dirty && faveoClientForm.descript@{{$index}}.$invalid">
                                          <span ng-show="faveoClientForm.descript@{{$index}}.$error.required">@{{node.label | translate}}
                                            is required.</span>
                </span>
                  <div style="width:85%" ng-if="node.type=='textarea'&&node.title=='Description'">
                    <textarea name="description" id="description@{{$index}}" class="form-control"
                              style="border-radius: 0px;" ng-model="node.value"
                              ng-required="@{{node.customerRequiredFormSubmit}}"></textarea>
                    ...
                    <div ng-if="node.title=='Description'" style="margin-top:20px">
                      <div id="fileList@{{$index}}"></div>
                        <span id="fileselector">
                            ...
                            <p><i>Max File Upload size : {{file_upload_max_size()}}MB and Max Number of Files
                                :{{ini_get('max_file_uploads')}}</i></p>
                        </span>
                    </div>
                  </div>
                  <div ng-if="node.type=='select'&&node.default=='yes'&&node.customerDisplay">
                    <select ng-model="node.value" name="selected@{{$index}}" id="seletom@{{$index}}"
                            ng-options="option.optionvalue for option in node.options" class="form-control"
                            style="border-radius: 0px;width:85%;display:inline-block"
                            ng-required="@{{node.customerRequiredFormSubmit}}"
                            ng-click="getSelectOptions(node.api,$index)">
                      <option value="">Select</option>
                    </select>
                    <span ng-show="loado@{{$index}}" style="width:15%"><img
                        src="{{asset("lb-faveo/media/images/gifloader.gif")}}" style="width:20px;height:20px"></span>
                    <div style="color:red"
                         ng-show="faveoClientForm.selected@{{$index}}.$dirty && faveoClientForm.selected@{{$index}}.$invalid">
                      <span ng-show="faveoClientForm.selected@{{$index}}.$error.required">@{{node.label | translate}} is required.</span>
                    </div>
                  </div>
                  <select ng-model="node.value" name="selected@{{$index}}"
                          ng-if="node.type=='select'&&node.default=='no'&&node.customerDisplay"
                          ng-options="option.optionvalue for option in node.options" class="form-control"
                          style="border-radius: 0px;width:85%" ng-required="@{{node.customerRequiredFormSubmit}}">
                    <option value="">Select</option>
                  </select>
                <span style="color:red"
                      ng-show="faveoClientForm.selected@{{$index}}.$dirty && faveoClientForm.selected@{{$index}}.$invalid">
                  ...
                  <ul class="list-group" ng-if="node.type=='radio'&&node.customerDisplay" style="border:none">
                    <li ng-repeat="option in node.options" class="list-group-item" style="border:none">
                      <input type="radio" name="selection" id="happy@{{$index}}" ng-model="node.value"
                             value="@{{option.optionvalue}}" ng-required="!node.value"/>
                  ...
                  <ul class="list-group" ng-if="node.type=='checkbox'" style="border:none">
                    <label ng-repeat="option in node.options" class="list-group-item" style="border:none">
                      ...
                      <ul ng-model="option.nodes" ng-class="{hidden: collapsed}"
                          style="list-style-type:none;margin-left: -70px"
                          ng-if="option.checked==true && option.nodes.length>0">
                        <li ng-repeat="node in option.nodes" ng-include="'nodes_renderer2.html'"></li>
                      </ul>
                    </label>
                  </ul>
                </div>
                <div class="col-sm-12" ng-repeat="option in node.options"
                     ng-if="option.nodes.length>0 && node.value && node.title=='Nested Select'">
                  <ul ng-model="option.nodes" ng-class="{hidden: collapsed}"
                      style="list-style-type:none;margin-left: -70px" ng-if="option==node.value">
                ...
                <div class="col-sm-12" ng-repeat="option in node.options"
                     ng-if="option.nodes.length>0 && node.value && node.title=='Nested Radio'">
                  <ul ng-model="option.nodes" ng-class="{hidden: collapsed}"
                      style="list-style-type:none;margin-left: -70px" ng-if="option.optionvalue==node.value">
                    <li ng-repeat="node in option.nodes" ng-include="'nodes_renderer2.html'">
                    </li>
                  </ul>
                </div>
          ...
          <div class="col-sm-8" style="border: 1px solid gainsboro;" id="rtl">
            <form name="faveoClientFormParent">
              ...
              <div class="row">
                <div class="col-sm-12"
                     style="border-top:1px solid gainsboro;background-color: white;padding:5px;text-align: right">
                  <button type="button" class="btn btn-info" ng-disabled="faveoClientFormParent.$invalid"
                          ng-click="uploadPic($event,requesterName)">Submit
                  </button>
                </div>
              </div>
            </form>
